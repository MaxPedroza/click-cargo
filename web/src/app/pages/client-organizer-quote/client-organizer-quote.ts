...
@Component({
  selector: 'app-client-organizer-quote',
  standalone: true,
  imports: [CommonModule, FormsModule, ValidationPopupComponent],
  template: `
    <div class="page">
      <h2>Solicitar cotação Personal Organizer</h2>
      <p class="hint">
        Atenção: fluxo demonstrativo de solicitação de Personal Organizer. O atendimento é apenas simulado.
      </p>
      <form (ngSubmit)="submit()" class="form">
        ...
        <label>
          Descreva o ambiente / necessidade de organização
      ...
      <cc-validation-popup
        ...
        title="Campos obrigatórios da solicitação"
        description="Para enviar a cotação de Personal Organizer, preencha:"
      ></cc-validation-popup>
    </div>
  `,
  styles: `
...
export class ClientOrganizerQuoteComponent {
  name = '';
  details = '';
  message = '';
  ...
  async submit() {
    ...
    if (!this.details) missing.push('Descrição do ambiente / necessidade');
    ...
    const res = await fetch(`${API_URL}/service-requests`, {
      ...
      body: JSON.stringify({ type: 'organizer', clientName: this.name, details: this.details }),
    });
    if (res.ok) {
      this.message = 'Solicitação de Personal Organizer registrada!';
      this.name = '';
      this.details = '';
    }
  }
